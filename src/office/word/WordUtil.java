...
public class WordUtil {
	public static void main(String[] args) throws Exception {
		exportWord("d:\\案件报告.docx");
	}
	
	public static void exportWord(String filePath) throws Exception {  
	 ...
	 * @param xdoc
	 */
    public static void createTable(XWPFTable xTable){  
        ...
        //表头
        setCellText(getCellHight(xTable, 0, 0), "序号",bgColor,1000);  
        setCellText(getCellHight(xTable, 0, 1), "阶段",bgColor,3800);  
        setCellText(getCellHight(xTable, 0, 2), "计划工作任务",bgColor,3800);  
        int number=0;  
        for(int i=1;i<5;i++){  
            number++;  
            setCellText(getCellHight(xTable, number,0), number+"",bgColor,1000);  
            setCellText(getCellHight(xTable, number,1), "階段名稱",bgColor,3800);  
            setCellText(getCellHight(xTable, number,2), "任務名稱",bgColor,3800);  
        }  
    }  
     ...
     * @throws IOException 
     */  
    public static void createSimpleTable(XWPFTable xTable)  
            throws IOException {  
        ...
        tblWidth.setType(STTblWidth.DXA);  
        for (int i = 0; i < 7; i++) {  
            setCellText(getCellHight(xTable, i, 0), "阶段", bgColor,3300);  
            setCellText(getCellHight(xTable, i, 1), "階段名稱", bgColor,3300);  
        }  
    }  
